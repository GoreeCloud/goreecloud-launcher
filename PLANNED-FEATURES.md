...
| --- | --- | --- | --- |
| `LCH-P0-01` | HOME return and workspace gesture stabilization: preserve primary-Home return, reliable content-origin drawer gesture handling, direct Home interactions, and smooth state transitions without whole-root recreation. | P0 / High | **Partial / acceptance open.** Primary-Home handling, configurable Home gestures, Room-authoritative spatial placement, long-press edit mode, unified Home↔Dock drag/drop, Dock and Home reordering, App Drawer copy-to-Home/Dock placement, widget-aware Home rendering, first-run startup wizard, replayable Home hints, explicit No apps / 10 most recent / 10 most used automatic Home modes, and transition diagnostics are implemented in Development. Automatic suggestions are transient and must not overwrite manual spatial placement. Open acceptance includes representative-device first-run flow, Home-role handoff, each automatic mode, manual drag/drop coexistence, hint accessibility/dismiss/replay, visual/gesture/one-handed behavior, and sustained performance under issue #80. |
