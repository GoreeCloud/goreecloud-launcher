...
| `LCH-P0-05` | Performance and Android system integration: shared icon cache/preloading, reduced recomposition/bitmap churn, transition profiling, responsive Home/Apps/Search entry, and Quickstep/Recents compatibility. | P0 / High | **Partial.** Shared caching, single-flight loading, bounded warming, invalidation guards, transition diagnostics, and async Search paths are implemented. Open: representative physical-device/default-HOME frame-time/jank/memory/power acceptance and Quickstep/Recents compatibility on supported Android/LineageOS boundaries. |
...
- Connect PR #199's persisted provider-control state to the rendered management experience without broadening its data boundary.
...
- Collect representative physical-device/default-HOME latency, frame-time/jank, memory, power, first-open/warm-open, and OEM-specific evidence.
