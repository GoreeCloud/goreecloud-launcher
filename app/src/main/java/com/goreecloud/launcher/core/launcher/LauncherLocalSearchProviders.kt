...
object LauncherLocalSearchProviderRegistry {
    fun registrations(context: Context): List<LauncherSearchProviderRegistration> = listOf(
        registration(
            LauncherShortcutsSearchProvider(context),
            LauncherSearchAuthorizationRequirement.NONE,
        ),
        registration(
            LauncherContactsSearchProvider(context),
            LauncherSearchAuthorizationRequirement.USER_CONSENT,
        ),
        registration(
            LauncherCallHistorySearchProvider(context),
            LauncherSearchAuthorizationRequirement.USER_CONSENT,
        ),
        registration(
            LauncherMessagesSearchProvider(context),
            LauncherSearchAuthorizationRequirement.USER_CONSENT,
        ),
    )
...
class LauncherShortcutsSearchProvider(context: Context) :
    ...
    override suspend fun searchAsync(
        ...
        if (rawQuery.isBlank() || !launcherApps.hasShortcutHostPermission()) {
        ...
        launcherApps.profiles
            .flatMap { user ->
                runCatching { launcherApps.getShortcuts(query, user).orEmpty() }
    ...
    private fun ShortcutInfo.toSearchResult(rawQuery: String): LauncherSearchResult? {
        ...
        return LauncherSearchResult(
            ...
            score = score + SHORTCUT_SCORE_BIAS,
    ...
    companion object {
        ...
        private const val SHORTCUT_SCORE_BIAS = 20
    }
}

class LauncherContactsSearchProvider(context: Context) :
    LauncherSearchProvider,
    LauncherAsyncSearchProvider {
    ...
    override suspend fun searchAsync(
        ...
        val rawQuery = request.rawQuery.trim()
        if (rawQuery.isBlank() || !LauncherLocalSearchPermissions.isGranted(appContext, id)) {
            return@withContext emptyList()
        }
...
class LauncherCallHistorySearchProvider(context: Context) :
    LauncherSearchProvider,
    LauncherAsyncSearchProvider {
    private val appContext = context.applicationContext
            ...
            while (cursor.moveToNext() && results.size < MAX_RESULTS) {
                ...
                results += LauncherSearchResult(
                    ...
                    action = LauncherOpenUriSearchAction(
                        ...
                        uri = Uri.fromParts("tel", number, null).toString(),
                    ),
                )
            }
        }
...
class LauncherMessagesSearchProvider(context: Context) :
    LauncherSearchProvider,
    LauncherAsyncSearchProvider {
    private val appContext = context.applicationContext
        ...
        appContext.contentResolver.query(
            ...
            while (cursor.moveToNext() && results.size < MAX_RESULTS) {
                ...
                results += LauncherSearchResult(
                    ...
                    action = LauncherOpenUriSearchAction(
                        intentAction = Intent.ACTION_VIEW,
                        uri = Uri.fromParts("sms", address, null).toString(),
                    ),
                )
            }
        }
