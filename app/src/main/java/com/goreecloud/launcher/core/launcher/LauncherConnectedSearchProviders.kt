...
data class LauncherConnectedSearchDefinition(
...
object LauncherConnectedSearchProviderRegistry {
    ...
    fun buildExplicitHandoffIntent(
        ...
        val definition = availableDefinitions(context)
            ...
            ?: return null
        return definition.buildIntent(query)
            .takeIf { intent -> resolves(context, intent) }
    }

    fun displayName(providerId: String): String? = definitions()
        .firstOrNull { it.providerId == providerId }
        ?.displayName
