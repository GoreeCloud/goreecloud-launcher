...
class WorkspaceProductionPromotionCoordinatorRuntimeTest {
    ...
    @Test
    fun verifiedWorkspaceCanPromoteThroughReviewedCoordinatorAndRemainHealthy() = runBlocking {
        ...
        assertEquals(
            WorkspaceProductionPromotionResult.NeedsVerification,
            coordinator(repository) { database.workspaceDao() }.promote(),
        )
    ...
    @Test
    fun divergenceAndUnavailableRoomBlockProductionPromotionWithoutChangingAuthority() = runBlocking {
        val repository = WorkspaceRepository(openWorkspaceDataStore())
        repository.ensureDefaults(
            favoriteKeys = INITIAL_FAVORITES,
            dockKeys = INITIAL_DOCK,
        ...
        assertEquals(
            WorkspaceProductionPromotionResult.Mismatch,
            coordinator(repository) { database.workspaceDao() }.promote(),
        )
        ...
        assertEquals(
            WorkspaceProductionPromotionResult.Unavailable,
            coordinator(repository) { null }.promote(),
        )
        assertEquals(WorkspaceAuthority.ROOM_VERIFIED, repository.state.first().authority)
    }
    ...
    ): WorkspaceProductionPromotionCoordinator = WorkspaceProductionPromotionCoordinator(
        repository = repository,
    ...
    private companion object {
        const val DATABASE_NAME = "launcher-workspace-production-promotion-test.db"
        const val WORKSPACE_DATASTORE_FILE = "launcher-workspace-production-promotion.preferences_pb"
