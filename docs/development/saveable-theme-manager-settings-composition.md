...
## GLAZE UI V1.3 mapping

`GlazeMetrics` records machine version `1.3.0`, release identity `Adaptive Resonance`, the exact Stable integration source revision, the V1.3 adaptive contract and Stable aggregate entrypoints, inherited spacing/radius values used by Launcher, inherited 8/16/24/32 dp optical geometry references, the 48 dp normal interaction floor, and the 56 dp Touch Assistance / far-view target.

`GlazeTheme` keeps System/Light/Dark/Deep Dark structural appearance support while applying Adaptive Resonance over the inherited V1.2 Frosted Neutral material foundation. `GlazeAtmosphere` contains neutral frosted material references, neutral degraded-effect outlines, and bounded accent references. It explicitly does not derive adaptive material color from wallpaper, application content, privacy/security state, or other producer truth.
...
The current preview uses inherited V1.2 Frosted Neutral structure with a single bounded Ice Blue accent marker. The accent sits inside the preview's cleared semantics tree and communicates no application state.
...
## V1.3 Adaptive Resonance and inherited Frosted Neutral authority boundary
...
Environmental sampling is not implemented. No wallpaper, application-content, privacy/security, or other producer-state sample is persisted, transmitted, or used to infer meaning. Removing transparency or decorative accent must leave meaning, actions, focus, state, and hierarchy intact.

## Historical evidence and rollback

V1.2 (`1.2.0`) is the immediate rollback baseline for this V1.3 migration. Earlier V1.2/V1.1/V1.0 and pre-reset Glaze UI 2.x source mappings and CI runs remain historical Development evidence only. They may support implementation ancestry but do not establish current V1.3 consumer acceptance or production eligibility.

## Remaining acceptance

This migration still requires complete V1.3 component/state/material/adaptive review, Reduced Motion, Reduced Transparency, Increased Contrast, native accessibility-equivalent/forced-color handling where applicable, 200% text/reflow, RTL/localization expansion, platform Touch Assistance resolution, representative phone/tablet/foldable behavior and reachability, TalkBack/Switch Access, performance fallbacks, Human Visual Excellence, representative-device Theme Manager navigation/persistence across all four modes, verified V1.2 rollback, production signing/distribution, release approval, and Release Candidate qualification.

This remains Development evidence only. A green build or correct token/semantics mapping does not establish complete GLAZE UI V1.3 conformance or production readiness.
